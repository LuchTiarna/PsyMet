...
#'@return Error
#'@export
likelihoodDeviance <- function(originalData, estimatedData, rmNaN=FALSE){
  error <- 0
    for(rowA in 1:nrow(originalData)){
      for(rowB in 1:nrow(estimatedData)){
        if(originalData[rowA,1]==estimatedData[rowB,1]){
          currentError <- estimatedData[rowB,2]*estimatedData[rowB,3]*log(estimatedData[rowB,3]/originalData[rowA,3])
          currentError <- currentError  + estimatedData[rowB,2]*(1-estimatedData[rowB,3])*log((1-estimatedData[rowB,3])/(1-originalData[rowA,3]))

          if(!rmNaN || !is.nan(currentError)){
            error <- error + 2*currentError
            }
          }
        }
      }
